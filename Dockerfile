...
# --prefer-offline + 长超时: npm 求解 rc 版全家桶的 peer 图较慢, 但不应无限回溯
...
#   从镜像内 /plugs/ vendor tgz 离线安装插件(见 package.json dependencies file: 引用)。
